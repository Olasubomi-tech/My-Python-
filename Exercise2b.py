...
for n in range(3):
    ...
    rel_errors_1 = []
    for m in m_interval:
        value = composite_quad(function, -0.5, 1.0, n, m)
        absolute_error = abs(value - Exact_integral)
        absolute_list_1.append(absolute_error)
        rel_err = abs(value - Exact_integral) / abs(Exact_integral)
        rel_errors_1.append(rel_err)
        
    beta, intercept, _, _, _ = linregress(np.log(h), np.log(rel_errors_1))
    alpha = np.exp(intercept)
    print(f"n = {n}: Estimated Alpha = {alpha:.4f}, Estimated beta = {beta:.4f} (Actual = {2*n+2})")
    plt.loglog(h, absolute_list_1, 'o-', label=f'n={n} (alpha = {alpha:.2f}, beta ≈ {beta:.2f})')
